...
from back.SpecialNodesFinalization import RemoveConstOps, CreateConstNodesReplacement, NormalizeTI
from moc_frontend.check_config import legacy_transformations_config_used, \
    tensorflow_custom_operations_config_update_used, new_extensions_used  # pylint: disable=no-name-in-module,import-error
from moc_frontend.pipeline import moc_pipeline  # pylint: disable=no-name-in-module,import-error
from moc_frontend.moc_emit_ir import moc_emit_ir  # pylint: disable=no-name-in-module,import-error
...
from pipeline.unified import unified_pipeline
from utils import import_extensions

# pylint: disable=no-name-in-module,import-error
from utils.cli_parser import check_available_transforms, \
...
from utils.get_ov_update_message import get_ov_update_message, get_ov_api20_message, \
    get_tf_fe_message, get_compression_message, get_ovc_message  # pylint: disable=no-name-in-module,import-error
from utils.get_ov_update_message import get_try_legacy_fe_message
from utils.model_analysis import AnalysisResults
from utils.version import VersionChecker
from utils.guess_framework import deduce_legacy_frontend_by_namespace
from utils.logger import init_logger, progress_printer  # pylint: disable=no-name-in-module,import-error
from utils.utils import refer_to_faq_msg, check_values_equal
from utils.telemetry_utils import send_params_info, send_framework_info, send_conversion_result, \
    init_mo_telemetry
from moc_frontend.check_config import legacy_extensions_used  # pylint: disable=no-name-in-module,import-error
from moc_frontend.pytorch_frontend_utils import get_pytorch_decoder, extract_input_info_from_example  # pylint: disable=no-name-in-module,import-error
from moc_frontend.paddle_frontend_utils import paddle_frontend_converter  # pylint: disable=no-name-in-module,import-error
from moc_frontend.shape_utils import parse_input_shapes  # pylint: disable=no-name-in-module,import-error
...
def load_extensions(argv: argparse.Namespace, is_tf: bool, is_caffe: bool, is_mxnet: bool, is_kaldi: bool,
    ...
    if is_tf:
        from front.tf.register_custom_ops import get_front_classes
        import_extensions.load_dirs(argv.framework, extensions, get_front_classes)
    elif is_caffe:
        send_framework_info('caffe')
        from front.caffe.register_custom_ops import get_front_classes
        import_extensions.load_dirs(argv.framework, extensions, get_front_classes)
    elif is_mxnet:
        send_framework_info('mxnet')
        from front.mxnet.register_custom_ops import get_front_classes
        import_extensions.load_dirs(argv.framework, extensions, get_front_classes)
    elif is_kaldi:
        send_framework_info('kaldi')
        from front.kaldi.register_custom_ops import get_front_classes
        import_extensions.load_dirs(argv.framework, extensions, get_front_classes)
    elif is_onnx:
        send_framework_info('onnx')
        from front.onnx.register_custom_ops import get_front_classes
        import_extensions.load_dirs(argv.framework, extensions, get_front_classes)
# DIT --- [2024/02/27, Kelvin]
...
def emit_ir(graph: Graph, argv: argparse.Namespace, non_default_params: dict):
    ...
    if not (argv.framework == 'tf' and argv.tensorflow_custom_operations_config_update):
        try:
            from back.offline_transformations import apply_offline_transformations  # pylint: disable=no-name-in-module,import-error
