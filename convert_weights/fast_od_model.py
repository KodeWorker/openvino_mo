...
from od_model import _conv2d_fixed_padding, _fixed_padding, _get_size, \
...
def fast_od_model(inputs, num_classes, is_training=False, data_format='NCHW', reuse=False):
    """
    Creates Fast Object Detection model.
    ...
    with slim.arg_scope([slim.conv2d, slim.batch_norm, _fixed_padding, slim.max_pool2d], data_format=data_format):
        with slim.arg_scope([slim.conv2d, slim.batch_norm, _fixed_padding], reuse=reuse):
            with slim.arg_scope([slim.conv2d],
                ...
                with tf.variable_scope('fast-od-model'):
                    #CSP BEGIN
