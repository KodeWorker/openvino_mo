...
def _res_Block(inputs,in_channels,res_num,data_format,double_ch=False):
...
def _conv_block(net,in_channels,a,b):
    for _ in range(a):
        out_channels=in_channels/2
        net = _conv2d_fixed_padding(net,out_channels,kernel_size=1)
...
def _csp_53(inputs,data_format,batch_norm_params):
    """
    Builds CSP-53 model.activation_fn=lambda x: tf.nn.leaky_relu(x, alpha=_LEAKY_RELU)
    """
    with slim.arg_scope([slim.conv2d], normalizer_fn=slim.batch_norm,
        ...
        #downsample
        #res1
        net=_res_Block(net,32,1,data_format,double_ch=True)
        #res2
        net = _res_Block(net,64,2,data_format)
        #res8
        net = _res_Block(net,128,8,data_format)

        #features of 54 layer
        up_route_54=net
        #res8
        net = _res_Block(net,256,8,data_format)
        #featyres of 85 layer
        up_route_85=net
        #res4
        net=_res_Block(net,512,4,data_format)
...
def od_model(inputs, num_classes, is_training=False, data_format='NCHW', reuse=False):
    """
    Creates Object Detection model.
    ...
    with slim.arg_scope([slim.conv2d, slim.batch_norm, _fixed_padding], data_format=data_format, reuse=reuse):
        ...
        with tf.variable_scope('csp-53'):
            route_1, route_2, route_3 = _csp_53(inputs,data_format,batch_norm_params)

        with slim.arg_scope([slim.conv2d], normalizer_fn=slim.batch_norm,
                            normalizer_params=batch_norm_params,
                            biases_initializer=None,
                            activation_fn=lambda x: tf.nn.leaky_relu(x, alpha=_LEAKY_RELU)):
            with tf.variable_scope('od-model'):
                #features of y1
